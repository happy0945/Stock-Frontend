...
const RegisterPage = () => {
  ...
  const handleRegister = async (e) => {
    ...
    const res = await dispatch(emailRegister({
      email:       form.email,
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.card}>
        ...
        <p className={styles.sub}>Join the live market terminal</p>

        {displayError && (
          <div className={styles.errorBanner}>
            <span>⚠</span> {friendlyError(displayError)}
          </div>
        )}

        {/* Social */}
        <div className={styles.socialGrid}>
          <SocialBtn provider="google"  icon={<GoogleIcon />}  label="Google"  onClick={() => handleSocial("google")}  disabled={loading} />
          <SocialBtn provider="github"  icon={<GithubIcon />}  label="GitHub"  onClick={() => handleSocial("github")}  disabled={loading} />
          <SocialBtn provider="twitter" icon={<TwitterIcon />} label="Twitter" onClick={() => handleSocial("twitter")} disabled={loading} />
        </div>
...
const StrengthBar = ({ password }) => {
  ...
  const colors = ["", "#ff4f6a", "#f5c842", "#00aaff", "#00d46e"];
  return (
